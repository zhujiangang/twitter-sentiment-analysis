...
//forse dovrei serializzare l'intero oggetto invece che solo classifier
//generico classificatore weka
public class WekaClassifier implements IClassifier, Serializable {
	...
	private Instance _toClassify;
	
	public WekaClassifier() throws Exception {
		DataSource source_train = new DataSource("D:/progetto/train1.arff");
		DataSource source_test = new DataSource("D:/progetto/test1.arff");
		_train = source_train.getDataSet();
		_test = source_test.getDataSet();
		/*_toClassify = new Instance(_train.numAttributes());
		int j = 0;
		while(j<_train.numAttributes()) {
			_toClassify.setValue(j, "0");
			j++;
		}*/
	}
	...
	public String classify(String stringa) throws FileNotFoundException,
		...
		//Instance inst = (Instance)_toClassify.copy();
